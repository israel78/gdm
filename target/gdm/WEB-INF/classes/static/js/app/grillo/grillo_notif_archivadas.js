	...
	datatable_setConfig: function(o_html_table, json_datos) {
		...
		this.o_datatable = o_html_table.DataTable({
	        ...
	        columns: [
				...
				,{title: "Secuencia", data: "secuencia", render: this.col_render_secuencia}
				,{title: "Tipo_notif.", data: "id_notificacion"} //ej: ESCALADO, CALCULADORA, ...Tipo notificación
				,{title: "F.Notificación", data: "f_notificar"}
				,{title: "Descripción", data: "txt_notificacion"} //son datos <HTML> !!!!
				,{title: "Leído_por", data: "id_visto"} //5
				//,{title: "¿Leído?", data: "id_visto", render: this.col_render_Leido} //6 //icono leído/no_leído. //Centrar cono: style="text-align: center;" 
			...
			*/
			...
			ajax_noSeUsa_sePideEn_grillo_js: {
				type: "POST", 
				url: "grillo.notificacion.ajax-data.action",
		...
		/* Comentado, los datos  se refrescan en "this.setJsonData" y se piden por AJAX en "grillo.js"
...
//	},
	...
	//datatable_createdCell: function (td, cellData, rowData, row, col) {
		...
		*/
		/*
		//$(grillo_notif.o_datatable.column(0).nodes()).addClass("gri_column"); //URL: https://datatables.net/examples/api/highlight.html
		$(grillo_notif.o_datatable.column(col).nodes()).addClass("gri_column"); //URL: https://datatables.net/examples/api/highlight.html
		*/
	//},
	//datatable_createdCell: function (td, cellData, rowData, row, col) {
	//
	//col_render_Leido: function (data, type, row, meta) {
	//	if ( (type != 'display') ) {
	//		return data;
	//	}
	...
	ajax_error: function(xhr, error, thrown) {
        // Make your callback here.
        /*
        //alert("grillo_notif.ajax_error(): AJAX ERROR!!!");	                
        console.log("grillo_notif.ajax_error(): AJAX ERROR!!!");	                
        console.log(xhr);	                
        console.log(error);	                
        console.log(thrown);
        */
        console.error("grillo_notif.ajax_error(): AJAX ERROR!!!");	                
        console.error("ERROR: " + error);
        console.error("SyntaxError.message: " + thrown.message);
	},
	//ajax_error: function(xhr, error, thrown) {
	
	// Establece los nuevos datos JSON a mostrar en la tabla: lo saca de 
	setJsonData: function(jsonData) {
		...
		*/
		...
		this.updateTotalNotificacionesLeidas(jsonData);
	},
	//
	// Muestra en el icono de la campana del menú superior el total de notificaciones no leídas
	updateTotalNotificacionesLeidas: function(jsonData) {
		...
		for (var i=0, len=jsonData.length; (i < len); i++) {
			...
			if (notificacion.id_visto) { //si tiene "id_visto"(=usu_que_marco_leida)...
				i_total_no_leidas++;
			}
		}
		
		$("#idSpanTotalNotificacionesArchivadas").text(i_total_no_leidas);
	},
...
//		o_waitDialog.setSize(BootstrapDialog.SIZE_WIDE);
		...
		*/
//		var s_action = "grillo.inc_mcn.notificacion.marcar.leido.action";
		//$.post( "notificacion.marcar.leido.action", v_data)
...
//					o_datatable.cell(cell.index().row, (cell.index().column - 1)).data(s_id_visto).draw(); //col "Leído_por"
					...
					//
					// Mostramos el total de notificaciones no leídas
					//grillo_notif.updateTotalNotificacionesLeidas(grillo_notif.o_datatable.data().toArray());
