	...
	datatable_setConfig: function(o_html_table, json_datos) {
		...
		this.o_datatable = o_html_table.DataTable({
	        ...
	        columns: [
	        	...
	        	//private String incidencia;		// secuencia
//	        	private String matricula;		// id_sesion
//	        	private String descripcion;		// dsc_seguimiento
//	        	private Date fechaSeguimiento; 	// f_seguimiento
//	        	private boolean estaActivo;		// activo
				{title: "", data: "_num_row", defaultContent: ""}  //0 //nº de fila. nº de notificación: 1..n
				,{title: "Incidencia", data: "incidencia"}
				,{title: "Descripcion.", data: "descripcion"} //ej: ESCALADO, CALCULADORA, ...Tipo notificación
				,{title: "Fecha de Seguimiento", data: "fechaSeguimiento"}
				,{title: "¿Activo?", data: "estaActivo"} //son datos <HTML> !!!!
		...
		.done(function( data, textStatus, jqXHR ) {
			...
			if ( (data && data.resultPeticion) ) {
				...
				/*	cell.data(s_id_visto).draw(); //col "¿Leído?"
					...
					//
					// Mostramos el total de notificaciones no leídas
					//grillo_seguimiento.updateTotalSeguimiento(grillo_seguimiento.o_datatable.data().toArray());
//	-				var total_seguidas = 0 + $("#idTotalSeguimiento").text();
//	-				if (s_id_visto) { 
//	-					total_seguidas--; //tenemos una no_leída menos 
//	-				} else  {
//	-					total_seguidas++; //tenemos una no_leída más
//	-				}
					$("#idTotalSeguimiento").text(total_seguidas);
				}
			} else {
